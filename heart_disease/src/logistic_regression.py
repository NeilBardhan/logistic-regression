...
#    true_pos = test[test['predictions'] == 1].shape[0] - false_pos
    ...
    accuracy = round(accuracy_score(test['target'], test['predictions']), 4)
    report = classification_report(test['target'], test['predictions'])
...
def main():
    ...
    baseline_accuracy, baseline_report = logistic_metrics(test, pred)
